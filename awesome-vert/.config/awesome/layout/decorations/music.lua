...
playerctl:connect_signal("metadata", function(_, title, artist, album_path, _, __, ___)
    ...
    song_name:set_markup_silently(
        helpers.colorize_text(title, beautiful.ext_light_fg or beautiful.fg_color)
    )
    song_artist:set_markup_silently(
        helpers.colorize_text(artist, beautiful.ext_light_fg or beautiful.fg_color)
    )
end)
...
local music_init = function(c)
    ...
    awful
        .titlebar(
            c,
            { position = "bottom", size = dpi(80), bg = beautiful.ext_light_bg_2 or beautiful.bg_2 }
        )
        :setup({
            pbar,
            {
